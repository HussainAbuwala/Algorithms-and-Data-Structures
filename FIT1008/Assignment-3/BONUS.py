...
import re
import os
from hash_table_task6 import QuadraticProbeTable
from hash_table_task6 import read_words_into_dictionary
from hash_table_task6 import frequency
from hash_table_task6 import read_file
...
class Special_Command_Line:
    def __init__(self):
        ...
        self.stack = Stack()
        self.dict = QuadraticProbeTable(402221)
        self.menu()
    ...
    def menu(self):
        ...
        valid_commands = ['insert', 'read', 'write', 'print',
                          'delete', 'frequency','fwa', 'undo', 'exit']
    ...
    def freq_word_analyzer(self):
        ...
        for i in range (len(words)):
            ...
            if words[i] not in duplicates:
                count = self.dict[words[i]]
                common_cal = self.dict.max / 100
                uncommon_cal = self.dict.max / 1000

                if count >= common_cal:
                    print(format_str.format(str(words[i])) + ' : ' + ' COMMON')
                elif count >= uncommon_cal:
                    print(format_str1.format(str(words[i])) + ' : ' + ' UNCOMMON')
                else:
                    print(format_str2.format(str(words[i])) + ' : ' + ' RARE')
